...
const adminNickname = cryptoJS.AES.encrypt('AdminBot', key).toString();
...
io.on('connection', socket => {
    ...
    socket.on('userJoined', (dataUserFront, cb)=> {
        ...
        socket.broadcast
            ...
            .emit('newMessage', createObjForMsg(adminNickname, `${dataUserFront.name} in room `, socket.id, 'received', adminAvatar))
    })
    /**
    ...
    socket.on('createMessage', (data, cb) => {
        ...
        console.log(data);
        const message = {
            name: cryptoJS.AES.encrypt(user.name, key).toString(),
            text: data.text,
            id: data.id,
            type: 'received',
